...
**技术说明：** `-t` / `--test` 标志由 `argparse` 解析，并将 `test_mode=True` 传入 `MainWindow`。在此模式下：

- **连接自检**（`ConnectionTester`）会在完成基础的 ping 与数据端口检查后，跳过 Moonraker / Klipper 服务检查，并直接报告成功，从而无需运行中的打印平台即可进入主界面。
- **相机与红外热像仪**的初始化失败会被捕获，因此即使没有实体硬件，GUI 仍能正常打开；但此时不会有实时的相机画面或红外数据。

简言之，测试模式让你在不接任何硬件的情况下打开并浏览界面，但实时硬件数据不可用。
