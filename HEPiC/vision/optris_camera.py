...
try:
    ...
    logger.error(
        "导入 Optris SDK 失败。请确保: 1) Optris SDK 已安装; "
        "2) Python 绑定路径已正确添加到 sys.path 或 PYTHONPATH; "
        "3) Python 版本与 SDK 绑定兼容。",
        exc_info=True,
    )
    OPTRIS_LIB_LOADED = False
except Exception:
    logger.exception("加载 Optris SDK 时发生意外错误")
    OPTRIS_LIB_LOADED = False
    ...
    class OptrisCamera(otc.IRImagerClient):
        ...
        def list_available_ranges(serial_number=0):
            ...
            if not OPTRIS_LIB_LOADED:
                logger.error("Optris SDK 未加载，无法查询范围。")
                return []
...
def main():
    ...
    if not OPTRIS_LIB_LOADED:
        logger.error("无法运行示例，Optris SDK 未加载。")
        return
...
if __name__ == "__main__":
    ...
    if OPTRIS_LIB_LOADED:
        ...
        logger.error("由于 Optris SDK 加载失败，无法运行 main() 函数。")
